...
보세요. 소셜 로그인은 커스텀 스킴(`timeflower://`)을 쓰므로 Expo Go가 아닌
...
브랜드는 **TimeFlower**, 색은 **따뜻한 베이지 지면 위의 테라코타**입니다. 앱 아이콘(열두
장 꽃잎)과 `theme.ts`가 같은 값을 씁니다 — 아이콘은 `npm run icons`로 다시 만듭니다.
값을 고칠 때는 [`docs/design-notes.md`](docs/design-notes.md) 18장의 대비 표를 함께
갱신하세요. 지면이 흰색이 아니라 여유가 얇습니다.

UI 시안은 별도로 끝나 있습니다 — 필수·중요 범위 16화면과 라벨 팔레트 12색.
[`docs/design/ui-proposal.html`](docs/design/ui-proposal.html)을 브라우저로 열면 봅니다.
**시안은 옛 이름·옛 색(살구)으로 그려져 있습니다.**

> **라벨 팔레트는 아직 8색입니다.** 시안의 12색(`docs/design-decisions.md` 5.3)과 크롬
> 토큰(5.2)은 남아 있습니다. 라벨 색은 브랜드가 아니라 데이터 부호라 테마를 따라가지
> 않으므로 이번 색 변경과 분리했습니다.

## 남은 것

설계안 11장의 1~8단계는 모두 구현했습니다. 출시 전에 필요한 것은 세 가지입니다.

- **라벨 12색 팔레트와 크롬 토큰** — 위 참고. 화면 코드는 거의 손대지 않습니다.
- **알림 발송 워커** — 큐는 쌓이지만 보내는 쪽이 없습니다. EAS 프로젝트와 개발
  빌드가 있어야 실제 동작을 확인할 수 있습니다.
- **유니버설 링크** — 초대 링크가 아직 `timeflower://` 스킴이라 메신저에서 눌리지
