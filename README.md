...
- 게스트 → 이메일·Google·Apple 계정 전환 시 **쓰던 데이터 유지**
- 이미 있는 이메일·Google·Apple 계정으로 로그인 (게스트 기록은 따라오지 않는다고 화면에서 고지)
...
클라우드 프로젝트 연결은 `supabase/README.md`를 보세요. Google·Apple 로그인은 운영
Supabase에서 해당 공급자와 Manual Linking을 활성화한 환경에서만 사용할 수 있습니다.
필요한 OAuth 설정은 `docs/deployment.md`의 로그인 공급자 절을 따릅니다.
