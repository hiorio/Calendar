...
반복 전개·타임존·시간 보정 같은 순수 함수를 확인합니다. **31개 전부 통과.**
...
| `src/lib/` 의 계산 로직 (반복·타임존·시간) | `npm run test:unit` |
...
1단계 스키마·RLS·Auth → 2단계 캘린더·초대·구성원 → 3단계 일정 CRUD →
4단계 반복 일정 → 5단계 참여자·댓글 → **6단계 알림까지 완료.**

6단계는 **큐까지**입니다. 일정 변경과 댓글이 `notification_outbox`에 쌓이고 설정
화면도 동작하지만, 그것을 Expo 푸시로 실제 보내는 워커는 없습니다. 이 환경에서
확인할 수 없기 때문입니다(Expo Go는 원격 푸시 미지원, EAS projectId 없음).

다음은 7단계(활동 로그), 8단계(계정 삭제)입니다.
화면에 `"6단계"` 같은 표시가 남아 있는 자리가 그 단계에서 채워질 곳입니다.
