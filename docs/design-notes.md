...
`expo-sharing` config plugin은 외부 앱에서 TimeLine으로 보내는 Share Extension을 추가해
별도 target과 app group까지 관리하게 됩니다. 현재 필요한 것은 TimeLine에서 iOS 공유
...
일러스트는 기존 서비스 캐릭터를 복제하지 않은 TimeLine 전용 동물 장면이며
